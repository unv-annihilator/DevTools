...
namespace Devtools.Client.Menus {
    public class MenuItemSpinnerList<T> : MenuItemSpinner {
        ...
        public MenuItemSpinnerList(Client client, Menu owner, string label, List<T> list, int defaultIndex, bool modulus = false,
            int priority = -1) : base(client, owner, label, defaultIndex, 0, list.Count, 1, modulus, priority) {
            Data = list;
        }
