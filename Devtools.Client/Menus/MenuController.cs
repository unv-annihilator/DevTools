    ...
    public class MenuController : ClientAccessor {
        ...
        private async Task OnRenderTick() {
            try {
                ...
                if (_viewportMin <= idx - MaxItemsVisible - 1) _viewportMin = Math.Max(0, idx - MaxItemsVisible);
                ...
                foreach (var item in visibleItems.Where(p => (count >= _viewportMin) & (count++ <= _viewportMax))) {
                    ...
                    if (item is MenuItemCheckbox cbox) HandleCheckboxRender(cbox, yOffset, isSelected);
                    else if (item is MenuItemSpinner spin) HandleSpinnerRender(spin, yOffset, isSelected);
                    else HandleStandardRender(item, yOffset, isSelected);
        ...
        private void HandleSpinnerRender(MenuItemSpinner type, float yOffset, bool isSelected) {
            ...
            var text = new Text($"{(isSelected ? "< " : "")}{type.GetDisplay()}{(isSelected ? " >" : "")}",
                new PointF(xPos, yOffset - 0.012f), 0.25f, TextColor, Font.ChaletLondon, Alignment.Right);
            UiHelper.DrawText($"{(isSelected ? "< " : "")}{type.GetDisplay()}{(isSelected ? " >" : "")}",
                new Vector2(xPos - text.ScaledWidth / 1920f, yOffset - 0.01f), isSelected ? TextColor : ActiveItemColor,
        ...
        private void HandleCheckboxRender(MenuItemCheckbox item, float yOffset, bool isSelected) {
            ...
            if (!item.IsChecked.Invoke()) UiHelper.DrawRect(centerPosX, yOffset, innerScaleX, innerScaleY, ItemColor);

            UiHelper.DrawText(item.Label,
                new Vector2(MenuPosX - (MenuWidth / 2 - 0.003f - scaleX * 1.25f), yOffset - (ItemHeight / 2 - 0.0036f)),
                isSelected ? ActiveTextColor : TextColor, 0.3f);
        }

        private void HandleStandardRender(MenuItem item, float yOffset, bool isSelected) {
            UiHelper.DrawText(item.Label, new Vector2(MenuPosX - (MenuWidth / 2 - 0.003f), yOffset - (ItemHeight / 2 - 0.0036f)),
                isSelected ? ActiveTextColor : TextColor, 0.3f);
        }
