...
namespace Devtools.Client.Controllers {
    public class HudMenu : Menu {
        ...
        public HudMenu(Client client, Menu parent) : base("HUD Menu", parent) {
            var radar = new MenuItemCheckbox(client, this, "Hide Minimap") {IsChecked = API.IsRadarHidden};
            radar.Activate += () => {
        ...
        private class MenuItemHudComponent : MenuItemCheckbox {
            ...
            public MenuItemHudComponent(Client client, Menu owner, string label, HudComponent comp, bool isChecked = false, int priority = -1)
                : base(client, owner, label, isChecked, priority) {
                Component = comp;
            ...
            protected override Task OnActivate() {
                if (DisabledComponents.Contains(Component)) DisabledComponents.Remove(Component);
