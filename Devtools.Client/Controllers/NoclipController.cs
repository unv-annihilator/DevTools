...
namespace Devtools.Client.Controllers {
    public class NoclipController : ClientAccessor {
        ...
        private async Task OnTick() {
            try {
                if (Game.IsControlJustPressed(2, Control.SaveReplayClip)) IsEnabled = !IsEnabled;
                ...
                // Speed Control
                if (Game.IsControlPressed(2, Control.SelectPrevWeapon)) Speed = Math.Min(Speed + 0.1f, MaxSpeed);
                else if (Game.IsControlPressed(2, Control.SelectNextWeapon)) Speed = Math.Max(0.1f, Speed - 0.1f);

                var multiplier = 1f;
                if (Game.IsControlPressed(2, Control.FrontendLs)) multiplier = 2f;
                else if (Game.IsControlPressed(2, Control.CharacterWheel)) multiplier = 4f;
                else if (Game.IsControlPressed(2, Control.Duck)) multiplier = 0.25f;
                ...
                // Disable controls
                foreach (var ctrl in DisabledControls) Game.DisableControlThisFrame(2, ctrl);
