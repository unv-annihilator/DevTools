...
namespace Devtools.Client.Controllers {
    public class VehicleMenu : Menu {
        ...
        public VehicleMenu(Client client, Menu parent) : base("Vehicle Menu", parent) {
            ...
            var invincible = new MenuItemCheckbox(client, this, "Invincible", IsInvincible) {IsChecked = () => IsInvincible};
            invincible.Activate += () => {
            ...
            var invisible = new MenuItemCheckbox(client, this, "Invisible", IsInvisible) {IsChecked = () => IsInvisible};
            invisible.Activate += () => {
            ...
            foreach (VehicleColor color in Enum.GetValues(typeof(VehicleColor))) vehicleColors.Add(color);
            ...
            foreach (LicensePlateStyle s in Enum.GetValues(typeof(LicensePlateStyle))) styles.Add(s);
            ...
            var hasNeon = new MenuItemCheckbox(client, this, "Neon Lights", HasNeon) {IsChecked = () => HasNeon};
            hasNeon.Activate += () => {
                HasNeon = !HasNeon;
                return Task.FromResult(0);
            };
        ...
        private async Task OnTick() {
            try {
                ...
                if (HasNeon) Game.PlayerPed.CurrentVehicle.Mods.NeonLightsColor = NeonColor;
        ...
        private static async Task<bool> SpawnVehicle(Model model) {
            if (Game.PlayerPed.CurrentVehicle != null) Game.PlayerPed.CurrentVehicle.Delete();
        ...
        private class MenuItemVehicleLivery : MenuItemSpinnerInt {
            ...
            public MenuItemVehicleLivery(Client client, Menu owner, int priority = -1) : base(client, owner, "Liveries", 0, 0, 2, 1, true,
                priority) {
                ValueUpdate += val => {
            ...
            public VehicleSpawnMenu(Client client, Menu parent) : base("Vehicle Spawner", parent) {
                ...
                inputModel.Activate += async () => {
                    try {
                        ...
                        if (await SpawnVehicle(model)) UiHelper.ShowNotification($"~g~Success~s~: Spawned ~y~{modelName}");
                        else
        ...
        private class MenuItemVehicle : MenuItem {
            ...
            public MenuItemVehicle(Client client, Menu owner, Model model, string modelName, int priority = -1) : base(client, owner,
                modelName, priority) {
                Model = model;
            }
