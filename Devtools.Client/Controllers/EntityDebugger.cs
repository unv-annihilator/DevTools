    ...
    public class EntityDebugger : ClientAccessor {
        ...
        private async Task OnTick() {
            try {
                ...
                if (_trackingEntity == null && !_track) _trackingEntity = GetEntityInCrosshair();


                var color = DefaultCrosshair;
                if (_trackingEntity != null) color = ActiveCrosshair;

                UiHelper.DrawCrosshair(color);

                if (_trackingEntity != null) {
                    if (rightClick) _track = true;
        ...
        private Entity GetEntityInCrosshair() {
            ...
            if (!raycast.DitHit || !raycast.DitHitEntity || raycast.HitPosition == default) return null;
            return raycast.HitEntity;
        }

        private Vector3 CameraForwardVec() {
            if (Client.NoClip.IsEnabled) return Client.NoClip.CurrentCamera.UpVector;
            var rotation = (float) (Math.PI / 180.0) * Function.Call<Vector3>(Hash.GET_GAMEPLAY_CAM_ROT, 2);
            return Vector3.Normalize(new Vector3((float) -Math.Sin(rotation.Z) * (float) Math.Abs(Math.Cos(rotation.X)),
                (float) Math.Cos(rotation.Z) * (float) Math.Abs(Math.Cos(rotation.X)), (float) Math.Sin(rotation.X)));
        }
        ...
        private static string GetModelName(Model model) {
            var name = "";
            if (model.IsVehicle) name = Enum.GetName(typeof(VehicleHash), (VehicleHash) model.Hash) ?? "";

            if (model.IsProp) name = Enum.GetName(typeof(ObjectHash), (ObjectHash) model.Hash) ?? "";

            if (string.IsNullOrEmpty(name)) name = Enum.GetName(typeof(PedHash), (PedHash) model.Hash) ?? "Unknown";
            return name;
        }
    }
}